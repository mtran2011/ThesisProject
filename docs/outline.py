...
class Environment:
    ...
    Use this info to calculate PnL and a reward r. Combine these info into a tuple or a state s. Show (r,s) to agent. 
    Agent gives back an action/order. Pass this order to exchange to execute, get transaction cost from it. Use this to form a new (r,s) and so on
...
class StockExchange:
    ...
    Methods:
        report_stock_price(): report stock price to the trading environment
